...
class AlignmentNoPos(Alignment):
    """
    do not send pos information
    """
    def __init__(self, semantic_dict, confusion_dict, granularity: str = "word"):
        super().__init__(semantic_dict, confusion_dict, granularity=granularity)
    ...
    def get_sub_cost(self, a_seg, b_seg):
        ...
        if self.granularity == "word":
            semantic_cost = self._get_semantic_cost(a_seg[0], b_seg[0]) / 6.0
            char_cost = self._get_char_cost(a_seg[0], b_seg[0], a_seg[1], b_seg[1])
            return semantic_cost + char_cost
        else:
            semantic_cost = self._get_semantic_cost(a_seg[0], b_seg[0]) / 6.0
            if a_seg[0] in punct and b_seg[0] in punct:
                pos_cost = 0.0
...
if __name__ == "__main__":
    tokenizer = Tokenizer("word", segmented=True)
    sentence1 = "_ 学生 大概 做 飞机 去 北京。"
    sentence2 = "_ 学生 大概 坐 飞机 去 北京。"
